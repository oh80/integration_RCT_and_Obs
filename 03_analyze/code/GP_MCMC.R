...
main <- function(){
  ...
  # MCMC
  samples <- run_MCMC(X, Y, Z, ID, iter=200, burn_in=50)
  return(samples)
}
...
run_MCMC <- function(X, Y, Z, ID, iter=1000, burn_in=200){
  ...
  for(t in 1:iter){
    # g
    #l_g <- renew_l(l_g, g_O, g_R, X_O, X_R, sig_hyper, alpha_l, beta_l)
    l_g <- renew_l(l_g, g, X, sig_hyper, alpha_l, beta_l)
    g <- renew_g(X, Y, Z, l_g, sig, tau, b_O, Obs_flag)
    #g_R <- renew_g(X_R, Y_R, Z_R, l_g, sig, HTE=tau_R)
    #g_O <- renew_g(X_O, Y_O, Z_O, l_g, sig, HTE=tau_O+b_O)
    
    # tau
    #l_tau <- renew_l(l_tau, tau_O, tau_R, X_O, X_R, sig_hyper, alpha_l, beta_l)
    l_tau <- renew_l(l_tau, tau, X, sig_hyper, alpha_l, beta_l)
    #tau_O <- renew_tau(X_O, Y_O, Z_O, l_tau, sig, g_O, b=b_O)
    #tau_R <- renew_tau(X_R, Y_R, Z_R, l_tau, sig, g_R)
    tau <- renew_tau(X, Y, Z, l_g, sig, g, b_O, Obs_flag)
    
    # b
    l_b <- renew_l_b(l_b, tau[Obs_flag], X_O, sig_hyper, alpha_l, beta_l)
    b_O <- renew_b(X_O, Y_O, Z_O, l_b, sig, g[Obs_flag], tau[Obs_flag])
  ...
  samples <- list("l_g"=l_g_list, "l_tau"=l_tau_list, "l_b"=l_b_list,
                  #"g_O"=g_O_list, "g_R"=g_R_list, "tau_O"=tau_O_list,"tau_R"=tau_R_list,
