...
main <- function(){
  ...
  res_list <- foreach(i=1:num_repeat) %dopar% {
    source(here::here("01_data", "code", "1d_logexp_generate.R"))
    
    # data generation
    seed = i
    train_data <- generate_data(seed, nO, nR, sig, beta)
  ...
  res_list |> bind_df_list() |> save_result("1d_logexp")
}
